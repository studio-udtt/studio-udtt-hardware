...
#include <WebServer.h>   // 자체 HTML 웹서버 라이브러리 선언
...
void handleRoot() 
{
  ...
  html += "<title>공간의 숨결 - 대시보드</title>";
  html += "<meta http-equiv='refresh' content='3'>";
  html += "<style>";
  html += "body { font-family: 'Segoe UI', Arial, sans-serif; background-color: #f4f7f6; margin: 0; padding: 20px; color: #333; }";
  html += ".container { max-width: 900px; margin: 0 auto; background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); }";
  html += "h1 { text-align: center; color: #2c3e50; margin-bottom: 30px; font-size: 28px; border-bottom: 2px solid #eef2f3; padding-bottom: 15px; }";
  
  html += ".main-score { text-align: center; background: linear-gradient(135deg, #1abc9c, #2ecc71); color: white; padding: 30px 20px; border-radius: 10px; margin-bottom: 25px; box-shadow: 0 4px 10px rgba(26,188,156,0.2); }";
  html += ".main-score .title { font-size: 18px; text-transform: uppercase; letter-spacing: 1px; font-weight: bold; opacity: 0.9; margin-bottom: 5px; }";
  html += ".main-score .score { font-size: 58px; font-weight: bold; }"; 
  html += ".main-score .unit { font-size: 22px; font-weight: normal; opacity: 0.8; margin-left: 8px; }"; 

  html += ".grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(380px, 1fr)); gap: 20px; }";
  html += ".card { background: #ffffff; padding: 22px; border-radius: 10px; border-left: 6px solid #3498db; box-shadow: 0 2px 8px rgba(0,0,0,0.04); display: flex; flex-direction: column; justify-content: center; }";
  html += ".card.people { border-left-color: #9b59b6; }";
  html += ".card.motion { border-left-color: " + String(isMoving ? "#2ecc71" : "#e74c3c") + "; }";
  html += ".card.noise { border-left-color: #f1c40f; }";
  html += ".card.lux { border-left-color: #e67e22; }";
  html += ".label { font-size: 14px; color: #7f8c8d; text-transform: uppercase; font-weight: bold; margin-bottom: 6px; }";
  html += ".value { font-size: 26px; font-weight: bold; color: #2c3e50; }";
  html += "</style></head><body>";
  
  html += "<div class='container'>";
  html += "<h1>🍃 공간의 숨결 실시간 모니터링</h1>";
  
  // [5. 센서별 정규화]
  html += "<div class='main-score'>";
  html += "<div class='title'>✨ 실시간 공간 숨결 지수</div>";
  html += "<div class='score'>" + String(breathOfSpaceIndex) + "<span class='unit'>/ 100 점</span></div>";
  html += "</div>";
  ...
  html += "<div class='label'>👥 실시간 재실 / 누적 방문자</div>";
  html += "<div class='value'>" + String((int)currentPeopleCount) + " 명 / " + String((int)totalVisitorCount) + " 명</div>";
  html += "</div>";
  
  // [2. mmWave 움직임 감지]
  html += "<div class='card motion'>";
  html += "<div class='label'>🏃 공간 움직임 상태</div>";
  html += "<div class='value'>" + String(isMoving ? "활성화" : "비활성화") + "</div>";
  html += "</div>";
  
  // [3. MAX9814 공간 소음 단계 및 Peak-to-Peak]
  html += "<div class='card noise'>";
  html += "<div class='label'>🔊 공간 소음 단계 (P-P)</div>";
  html += "<div class='value'>" + soundStatus + " (" + String(globalPeakToPeak) + ")</div>";
  html += "</div>";
  
  // [4. BH1750 실시간 조도값]
  html += "<div class='card lux'>";
  html += "<div class='label'>☀️ 실시간 조도</div>";
  html += "<div class='value'>" + luxStatus + " (" + String((int)globalLux) + ")</div>";
  html += "</div>";
...
void setup()
{
  ...
  // [0. HTML 설정]
  server.on("/", handleRoot); // 사용자가 HTML 접속하면, handleRoot 가동
  server.begin();
  Serial.println("[🟢 시스템] 웹 서버 가동 완료");
  Serial.print("IP: ");
...
void loop()
{
  ...
  // [0. HTML 설정]
  server.handleClient(); // HTML 대시보드 열람 요청 시 동작
...
// [5. 공간의 숨결 데이터를 위한 센서별 정규화]
  ...
  // 2. 소음 점수화
  float scoreNoise = map(globalPeakToPeak, 50, 400, 0, 100);  // 공간 소음 정도에 맞게 조절 필요, 현재는 '활발함' 판단 기준인 400 기준
